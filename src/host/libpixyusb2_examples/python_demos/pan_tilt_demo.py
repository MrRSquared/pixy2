...
def Display_Block (Index, Block,P):
        print ('                   Block[%3d]: I: %3d / S:%2d / X:%3d / Y:%3d / W:%3d / H:%3d / A:%3d /P:%3d' % (Index, Block.m_index, Block.m_signature, Block.m_x, Block.m_y, Block.m_width, Block.m_height, Block.m_age,P))
...
class PID_Controller:
  ...
  def Reset (self):
    self.Previous_Error  = 0x7fffffff
  ...
  def Update (self, Error):
    ...
    if self.Previous_Error !=  0x7fffffff:
      ...
      PID = round (Error * self.Proportion_Gain + (round(self.Integral_Value * self.Integral_Gain) >> 4) + (Error - self.Previous_Error) * self.Derivative_Gain) >> 10;
      print (PID)
      if self.Servo:
